...
def test_bidirectional_sync_crud(session):
    ...
    cycle = Cycle(title="Test Cycle", start_date=utc_now_naive(), end_date=utc_now_naive())
    session.add(cycle)
    session.commit()
...
def test_full_hierarchy_aggregation(session):
    ...
    update_key_result(kr1.id, actor_username=user.username, current_value=50) # Direct value update

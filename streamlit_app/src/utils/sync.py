...
def _cleanup_stale_nodes(session, username, current_ids: set):
    ...
    goals = session.exec(select(Goal).where(Goal.owner_id == owner_id_subq)).all()
    ...
    def clean_model(model_class, parent_field, parent_ids):
        ...
        items = session.exec(
            select(model_class).where(
                getattr(model_class, parent_field).in_(parent_ids)
            )
        ).all()
        kept_ids = []
        for item in items:
...
def _sync_node(
    session, model_class, json_node, username, parent_id=None, all_nodes=None
):
    node_id = json_node.get("id")
    json_node.get("type", "").upper()
    ...
    if model_class == KeyResult:
        ...
        fields["initiative_tags"] = (
            json.dumps(json_node.get("initiative_tags", []))
            if isinstance(json_node.get("initiative_tags"), list)
            else json_node.get("initiative_tags", "[]")
        )

    # Goal specific
    if model_class == Goal:
        fields["cycle_id"] = json_node.get("cycle_id")
        fields["strategy_tags"] = (
            json.dumps(json_node.get("strategy_tags", []))
            if isinstance(json_node.get("strategy_tags"), list)
            else json_node.get("strategy_tags", "[]")
        )
    ...
    if sql_node:
        ...
        if parent_id is not None:
            if model_class == Objective:
                sql_node.goal_id = parent_id
            elif model_class == KeyResult:
                sql_node.objective_id = parent_id
...
def _sync_children(
    session, all_nodes, parent_json_node, parent_sql_id, child_type, username
):
    ...
    for cid in child_ids:
        ...
        sql_child = _sync_node(
            session,
            model_class,
            c_node,
            username,
            parent_id=parent_sql_id,
            all_nodes=all_nodes,
        )
        if sql_child:
            if next_type:
                _sync_children(
                    session, all_nodes, c_node, sql_child.id, next_type, username
                )
...
def _sync_work_logs(session, task_json_node, task_sql_id):
    ...
    for log in work_logs:
        ...
        statement = (
            select(WorkLog)
            .where(WorkLog.task_id == task_sql_id)
            .where(WorkLog.start_time == start_dt)
        )

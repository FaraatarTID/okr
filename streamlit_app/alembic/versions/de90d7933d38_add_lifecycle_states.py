...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        sa.Enum("DRAFT", "ACTIVE", "GRADING", "ARCHIVED", name="lifecyclestate").create(
            bind, checkfirst=True
        )
        sa.Enum("BOOLEAN", "NUMERIC", "PERCENT", name="metrictype").create(
            bind, checkfirst=True
        )
        sa.Enum("UNWEIGHTED", "WEIGHTED", name="scoremode").create(
            bind, checkfirst=True
        )
        ...
        op.execute(
            "ALTER TABLE key_result ALTER COLUMN metric_type SET DEFAULT 'NUMERIC'::metrictype"
        )
        op.execute("ALTER TABLE key_result ALTER COLUMN metric_type SET NOT NULL")
        op.execute(
            "ALTER TABLE objective ALTER COLUMN score_mode SET DEFAULT 'UNWEIGHTED'::scoremode"
        )
        ...
        if "key_result" in existing_tables:
            ...
            if "final_reflection" not in kr_cols:
                op.add_column(
                    "key_result",
                    sa.Column("final_reflection", sa.String(), nullable=True),
                )
...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        op.execute(
            "ALTER TABLE key_result ALTER COLUMN metric_type TYPE varchar USING metric_type::text"
        )
        op.execute(
            "ALTER TABLE key_result ALTER COLUMN metric_type SET DEFAULT 'numeric'::varchar"
        )
        op.execute(
            "ALTER TABLE objective ALTER COLUMN score_mode TYPE varchar USING score_mode::text"
        )
        op.execute(
            "ALTER TABLE objective ALTER COLUMN score_mode SET DEFAULT 'unweighted'::varchar"
        )

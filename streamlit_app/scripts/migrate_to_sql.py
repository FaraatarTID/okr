...
def migrate():
    ...
    json_files = [
        ...
        if f.startswith("okr_data_") and f.endswith(".json")
    ]
    ...
    for filename in json_files:
        ...
        for rid in root_ids:
            node = nodes.get(rid)
            if not node or node.get("type") != "GOAL":
                continue
            ...
            sql_goal = create_goal(
                ...
                created_at=datetime.fromtimestamp(node.get("createdAt", 0) / 1000)
            ...
            for oid in node.get("children", []):
                ...
                if not o_node or o_node.get("type") != "OBJECTIVE":
                    continue
                ...
                sql_obj = create_objective(
                    ...
                    created_at=datetime.fromtimestamp(o_node.get("createdAt", 0) / 1000)
                    if o_node.get("createdAt")
                    else None,
                    actor_username=username,
                )
                ...
                for kid in o_node.get("children", []):
                    ...
                    if not k_node or k_node.get("type") != "KEY_RESULT":
                        continue
                    ...
                    sql_kr = create_key_result(
                        ...
                        created_at=datetime.fromtimestamp(
                            k_node.get("createdAt", 0) / 1000
                        )
                        if k_node.get("createdAt")
                        else None,
                    ...
                    for tid in k_node.get("children", []):
                        ...
                        if not t_node or t_node.get("type") != "TASK":
                            continue

                        # Handle status
                        status = TaskStatus.TODO
                        if t_node.get("status") == "done":
                            status = TaskStatus.DONE
                        elif t_node.get("status") == "in_progress":
                        ...
                        sql_task = create_task(
                            ...
                            created_at=datetime.fromtimestamp(
                                t_node.get("createdAt", 0) / 1000
                            )
                            if t_node.get("createdAt")
                            else None,
                            deadline=datetime.fromtimestamp(
                                t_node.get("deadline", 0) / 1000
                            )
                            if t_node.get("deadline")
                            else None,
                            actor_username=username,
                        )
                        ...
                        for log in t_node.get("workLog", []):
                            ...
                            add_manual_log(
                                ...
                                note=log.get(
                                    "summary"
                                ),  # Summary from JSON to Note in SQL
                                log_date=log_date,
                                actor_username=username,
                            )

...
def test_compose_fails_closed_for_internal_credentials_and_gates_web_on_bff() -> None:
    ...
    assert (
        'test: ["CMD-SHELL", "wget -q -O - http://127.0.0.1:${BFF_PORT:-3001}/readyz'
        in text
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "OKR_BACKEND_API_URL=http://public.example:8100",
            "must target the private backend service",
        ),
    ],
)
def test_rejects_public_backend_or_database_ingress(

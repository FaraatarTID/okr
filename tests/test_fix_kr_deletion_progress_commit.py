...
def test_delete_key_result_commits_progress_recalculation(isolated_db):
    ...
    create_check_in(kr1.id, value=80, confidence=7, comment="KR1 progress", actor_username="alice", variation_type=VariationType.COMMON_CAUSE)
    create_check_in(kr2.id, value=40, confidence=5, comment="KR2 progress", actor_username="alice", variation_type=VariationType.COMMON_CAUSE)
...
def test_delete_key_result_updates_goal_progress(isolated_db):
    ...
    kr1 = create_key_result(objective.id, "KR A", target_value=100, actor_username="bob")
    kr2 = create_key_result(objective.id, "KR B", target_value=100, actor_username="bob")

    from src.crud import update_objective

    update_objective(objective.id, state=LifecycleState.ACTIVE, actor_username="bob")

    create_check_in(kr1.id, value=100, confidence=8, comment="Done", actor_username="bob", variation_type=VariationType.COMMON_CAUSE)
    create_check_in(kr2.id, value=50, confidence=5, comment="Half", actor_username="bob", variation_type=VariationType.COMMON_CAUSE)

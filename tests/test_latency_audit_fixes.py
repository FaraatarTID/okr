...
from src.models import User, Goal, Objective, KeyResult, Task, WorkLog
...
def _seed_basic_tree():
    from src.crud import create_user, create_goal, create_cycle, create_objective, create_key_result, create_task
    user = create_user("test_user", "pass")
    cycle = create_cycle("Cycle 1", start_date=_utc_now_naive()-timedelta(days=1), end_date=_utc_now_naive()+timedelta(days=30))
    goal = create_goal(user.username, "Test Goal", cycle_id=cycle.id, actor_username=user.username)
...
def test_cached_node_retrieval_logic(isolated_db, monkeypatch):
    ...
    count1 = _count_queries(isolated_db, lambda: components._cached_get_node(goal.id, "GOAL"))
    assert count1 > 0
    
    # Second call - should hit cache (0 queries)
    count2 = _count_queries(isolated_db, lambda: components._cached_get_node(goal.id, "GOAL"))
    assert count2 == 0
...
def test_cached_work_logs_retrieval(isolated_db, monkeypatch):
    ...
    count2 = _count_queries(isolated_db, lambda: components._cached_get_work_logs(task_id))
    assert count2 == 0

...
def test_extract_ai_snapshot_fields_handles_bad_score_with_debug_log():
    ...
    assert state == "risk"
    assert any("Failed to parse atlas AI overall score" in msg for msg in logger.debug_calls)
...
def test_get_node_details_from_lookup_prefers_explicit_lookup_then_session_state():
    node_type, title = atlas_runtime_lookup_helpers.get_node_details_from_lookup(
        ...
        session_state={"atlas_node_lookup": {"task_2": {"type": "TASK", "title": "Task Two"}}},
    )
    ...
    missing_type, missing_title = atlas_runtime_lookup_helpers.get_node_details_from_lookup(
        "task_404",
        node_lookup={},
        session_state={},
    )

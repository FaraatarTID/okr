...
def credential(principal: str = "operator-a") -> OperatorCredential:
    return OperatorCredential(
        principal=principal,
        credential_id=f"test:{principal}",
...
def sign_attestation(attestation: dict[str, object]) -> str:
    ...
    return (
        "hmac-sha256:"
        + hmac.new(ATTESTATION_SECRET.encode(), encoded, hashlib.sha256).hexdigest()
    )
...
def test_control_plane_default_path_reloads(monkeypatch, tmp_path: Path) -> None:
    ...
    first.update_environment_metadata(
        "env-integration", release_digest="sha256:" + "1" * 64
    )
    second = ControlPlane()
    assert (
        second.get_environment("env-integration").release_digest == "sha256:" + "1" * 64
    )


def test_control_plane_without_configured_path_does_not_write_local_state(
    ...
    assert (
        control_plane.get_environment("env-integration").environment_id
        == "env-integration"
    )
...
def test_provision_release_and_backup_reconcile_one_summary(tmp_path: Path) -> None:
    ...
    provider = LocalDisposableEnvironmentProvider()
    Provisioner(provider, operator=credential()).with_control_plane(
        control_plane
    ).provision(manifest())
    ...
    ).deploy("env-integration", artifact)
...
def test_local_release_and_backup_persistence_use_shared_lock(
...
def test_release_persistence_preserves_records_from_stale_process_instances(
    tmp_path: Path,
) -> None:
...
def test_backup_persistence_preserves_records_from_stale_process_instances(
    tmp_path: Path,
) -> None:
    ...
    assert {record["environment_id"] for record in reloaded._backups.values()} == {
        "env-a",
        "env-b",
    }


def test_phase_evidence_checker_fails_incomplete_and_passes_complete(
    tmp_path: Path,
) -> None:
...
def test_phase_evidence_checker_rejects_transplanted_bound_values(
    ...
    payload = {
        ...
        "release": {
            "artifacts": [
                {"version": "release-0", "digest": "sha256:" + "0" * 64},
                {"version": "release-1", "digest": "sha256:" + "1" * 64},
            ],
            "release_identity": "release-rehearsal-a",
            "rollback_result": "passed",
            "measured_rollback_seconds": 45,
...
```"""
    ...
    assert any(
        "signature" in error for error in check(evidence, secret=ATTESTATION_SECRET)
    )


def test_phase_evidence_checker_rejects_headings_without_structured_values(
    tmp_path: Path,
) -> None:
...
def test_phase_evidence_checker_rejects_synthetic_attestation_values(
    tmp_path: Path,
) -> None:

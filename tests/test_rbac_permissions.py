...
def test_rbac_matrix():
    ...
    manager = User(
        id=2, username="manager", role=UserRole.MANAGER, is_active=True, team_id=10
    )
    member = User(
        id=3, username="member", role=UserRole.MEMBER, is_active=True, team_id=10
    )
    outsider = User(
        id=4, username="outsider", role=UserRole.MEMBER, is_active=True, team_id=20
    )

    # Setup resources
    # Goal owned by member
    member_goal = Goal(
        id=100, owner_id=member.id, team_id=member.team_id, title="Member Goal"
    )
    # Goal owned by manager
    manager_goal = Goal(
        id=101, owner_id=manager.id, team_id=manager.team_id, title="Manager Goal"
    )
    # Goal owned by outsider
    outsider_goal = Goal(
        id=102, owner_id=outsider.id, team_id=outsider.team_id, title="Outsider Goal"
    )
...
def test_manager_can_update_direct_report_resource_with_session_lookup():
    ...
    assert (
        check_permission(manager, Action.UPDATE, member_goal, session=session) is True
    )

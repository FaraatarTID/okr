...
def test_login_navigate_atlas_map_and_start_timer(e2e_stack: E2EStack) -> None:
    ...
    with sync_playwright() as playwright:
        ...
        expect(page.get_by_label("Username", exact=True)).to_be_visible(timeout=60_000)
        page.get_by_label("Username", exact=True).fill(e2e_stack.username)
        page.get_by_label("Password", exact=True).fill(e2e_stack.password)

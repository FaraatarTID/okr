...
if not defined OKR_BACKEND_SERVICE_TOKEN for /f %%I in ('python -c "import secrets; print(secrets.token_hex(16))"') do set "OKR_BACKEND_SERVICE_TOKEN=%%I"
...
for /f %%I in ('python -c "import secrets; print(secrets.token_hex(16))"') do set "BFF_SESSION_SECRET=%%I"
...
for /f "usebackq delims=" %%R in (`python scripts\db_tcp_probe.py 2^>nul`) do set "DB_CONNECTIVITY_CHECK=%%R"
...
if not defined ALLOW_SQLITE_FALLBACK set "ALLOW_SQLITE_FALLBACK=false"

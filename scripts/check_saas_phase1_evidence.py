...
FORBIDDEN_MARKERS = re.compile(
    r"(?:local|test|synthetic|fake|dummy|fixture|mock)", re.IGNORECASE
)
...
def check(path: Path, *, secret: str | None = None) -> list[str]:
    ...
    errors: list[str] = []
    errors.extend(
        f"missing structured evidence field: {field}"
    ...
    if (
        evidence["decision"].get("status") != "APPROVED"
        or not str(evidence["decision"].get("owner", "")).strip()
    ):
        errors.append("decision approval and named owner are required")
    if (
        not str(evidence["provisioning"].get("environment_id", "")).strip()
        or evidence["provisioning"].get("idempotent") is not True
    ):
        errors.append(
            "provisioning environment_id and idempotency evidence are required"
        )
    artifacts = evidence["release"].get("artifacts", [])
    digest = re.compile(r"^sha256:[0-9a-f]{64}$")
    if len(artifacts) < 2 or any(
        not str(item.get("version", "")).strip()
        or not digest.fullmatch(str(item.get("digest", "")))
        for item in artifacts
    ):
        errors.append(
            "two immutable artifact versions with SHA-256 digests are required"
        )
    ...
    if (
        not isinstance(measured_rollback, (int, float))
        or isinstance(measured_rollback, bool)
        or measured_rollback < 0
    ):
        errors.append("measured rollback seconds are required")
    backup = evidence["backup"]
    if (
        not str(backup.get("provider", "")).strip()
        or not str(backup.get("backup_id", "")).strip()
    ...
    if (
        restore.get("result") != "passed"
        or not str(restore.get("target", "")).strip()
        or not str(restore.get("restore_id", "")).strip()
    ):
    ...
    if not isinstance(attestation, dict):
        ...
        errors.extend(
            f"missing attestation field: {field}"
            for field in sorted(REQUIRED_ATTESTATION_FIELDS - attestation.keys())
        )
        provider = str(attestation.get("provider", "")).strip()
        if not provider or FORBIDDEN_MARKERS.search(provider):
            errors.append(
                "explicit production provider is required; local/test providers are forbidden"
            )
        for field, label in (
            ("backup_id", "provider-issued backup ID"),
            ("restore_id", "provider-issued restore ID"),
        ):
            value = str(attestation.get(field, "")).strip()
            if (
                not value
                or not OPAQUE_ID.fullmatch(value)
                or FORBIDDEN_MARKERS.search(value)
            ):
                errors.append(
                    f"{label} is required and synthetic/test IDs are forbidden"
                )
        for field in (
            "environment_id",
            "customer_id",
        ...
        if (
            ...
            errors.append(
                "attested artifact digests must match the release artifact digests"
            )
        for field in (
            "measured_rollback_seconds",
            "measured_rpo_seconds",
            ...
            if (
                ...
                errors.append(f"attested {field} is required")
        if not _named(attestation.get("decision_owner")) or not _named(
            attestation.get("operations_owner")
        ):
            errors.append("attested named decision and operations owners are required")
        if (
            provider != str(backup.get("provider", "")).strip()
            or str(attestation.get("backup_id", "")).strip()
            != str(backup.get("backup_id", "")).strip()
        ):
            errors.append(
                "attested provider and backup ID must match structured backup evidence"
            )
        if (
            str(attestation.get("restore_id", "")).strip()
            != str(restore.get("restore_id", "")).strip()
        ):
            errors.append("attested restore ID must match structured restore evidence")
        if attestation.get("environment_id") != evidence["provisioning"].get(
            "environment_id"
        ):
            errors.append(
                "attested environment ID must match structured provisioning evidence"
            )
        if attestation.get("customer_id") != evidence["provisioning"].get(
            "customer_id"
        ):
            errors.append(
                "attested customer ID must match structured provisioning evidence"
            )
        if attestation.get("backup_target") != evidence["backup"].get("backup_target"):
            errors.append(
                "attested backup target must match structured backup evidence"
            )
        if attestation.get("release_identity") != evidence["release"].get(
            "release_identity"
        ):
            errors.append(
                "attested release identity must match structured release evidence"
            )
        if attestation.get("provisioning_identity") != evidence["provisioning"].get(
            "provisioning_identity"
        ):
            errors.append(
                "attested provisioning identity must match structured provisioning evidence"
            )
        if attestation.get("measured_rollback_seconds") != measured_rollback:
            errors.append(
                "attested rollback measurement must match structured release evidence"
            )
        if attestation.get("measured_rpo_seconds") != evidence["rpo_rto"].get(
            "measured_rpo_seconds"
        ) or attestation.get("measured_rto_seconds") != evidence["rpo_rto"].get(
            "measured_rto_seconds"
        ):
            errors.append(
                "attested RPO/RTO measurements must match structured evidence"
            )
        if attestation.get("decision_owner") != owners.get(
            "decision"
...
def _named(value: object) -> bool:
    ...
    return (
        bool(text)
        and text.upper() != "UNASSIGNED"
        and not FORBIDDEN_MARKERS.search(text)
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("docs/saas/phase-1-entry-evidence.md"),
    )

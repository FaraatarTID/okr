...
- Use this map to decide *where* code should live and *which tests* should be updated.
...
2. `streamlit_app/src/ui/components.py` exposes a stable UI facade and delegates to `src/ui/*_helpers.py`.
3. `streamlit_app/src/crud.py` exposes a stable domain/data facade and delegates to `src/crud_*_helpers.py`.
4. `streamlit_app/src/database.py` and `streamlit_app/src/models.py` own persistence contracts.
5. Optional backend path: `backend_app/main.py` (API) -> `backend_app/jobs.py` -> `backend_app/worker.py`.

...
def _validate_provider_response(
    ...
    *,
    ...
    if (
        ...
        and raw.get("environment_id") != expected_environment_id
    ):
        raise ProviderContractError(
            "provider response belongs to a different environment"
        )
...
def _validate_provider_for_environment(provider: Any, *, production: bool) -> None:
    ...
    if production and provider_name == LocalBackupProvider.provider_name:
        raise ProviderContractError(
            "a production provider is required; local adapter is test-only"
        )
...
def validate_restore_target(target: RestoreTarget) -> None:
    ...
    if target.live or not target.environment_id.strip():
        raise UnsafeRestoreTarget(
            "restore target must be explicitly registered and isolated"
        )
    value = target.database_target.strip()
    if not value or re.search(
        r"(^|[-_.:/])(live|prod|production)(?=$|[-_.:/])", value.lower()
    ):
        raise UnsafeRestoreTarget(
            "live or production-like restore targets are prohibited"
        )
...
class BackupProvider(Protocol):
    ...
    def create_backup(
        self, environment_id: str, retention_class: str
    ) -> dict[str, Any]: ...
...
class RestoreProvider(Protocol):
    ...
    def restore_backup(
        self, backup_id: str, target: RestoreTarget
    ) -> dict[str, Any]: ...
...
class BackupManager:
    ...
    def create(self, environment_id: str) -> BackupRecord:
        ...
        try:
            ...
            self.provider.record_status(
                f"create-failure:{environment_id}",
                {
        ...
        try:
            ...
            self.provider.record_status(
                backup_id,
                {
                    ...
                    "last_success_at": prior_status.get("last_success_at"),
                    "last_failure_at": now,
                    "failure_reason": str(failure),
                    "freshness_seconds": self._age(original.created_at, now),
                    "retention_class": original.retention_class,
                    "rpo_seconds": original.rpo_seconds,
                    "rto_seconds": original.rto_seconds,
                    "restore_test_status": prior_status.get(
                        "restore_test_status", "NOT_TESTED"
                    ),
                },
            )
        ...
        try:
            _validate_provider_response(
                ...
                expected_environment_id=(
                    known_record.environment_id
                    if known_record
                    else prior_status.get("environment_id")
                ),
            )
            ...
            self.provider.record_status(
                ...
                {
                    **prior_status,
                    "provider": raw.get("provider", self.provider.provider_name),
                    "backup_id": backup_id,
        ...
        self.provider.record_status(
            backup_id,
            {
                **prior_status,
                ...
                "failure_reason": None,
                "freshness_seconds": age,
                "retention_class": prior_status.get(
                    "retention_class", raw.get("retention_class", self.retention_class)
                ),
                "rpo_seconds": prior_status.get("rpo_seconds", self.rpo_seconds),
                "rto_seconds": prior_status.get("rto_seconds", self.rto_seconds),
                "restore_test_status": prior_status.get(
                    "restore_test_status", "NOT_TESTED"
                ),
            },
    ...
    def _reconcile_failure(self, environment_id: str, kind: str, reason: str) -> None:
        if self.control_plane is not None:
            self.control_plane.update_environment_metadata(
                environment_id, backup_state="failed", backup_verified=False
            )
...
class RestoreManager:
    ...
    def restore(self, backup_id: str, isolated_target: RestoreTarget) -> RestoreRecord:
        ...
        try:
            ...
            self.backup_provider.record_status(
                backup_id,
                {
                    **status,
                    "restore_test_status": "FAILED",
                    "restore_test_error": str(failure),
                    "restore_test_elapsed_seconds": max(
            ...
            if self.control_plane is not None:
                self.control_plane.update_environment_metadata(
                    status.get("environment_id", isolated_target.environment_id),
                    backup_state="restore-failed",
                    backup_verified=False,
                )
            raise
        ...
        self.backup_provider.record_status(
            backup_id,
            {
                **status,
                ...
                "last_restore_test_at": datetime.now(UTC).isoformat(),
                "last_restore_target": isolated_target.database_target,
                "last_restore_elapsed_seconds": elapsed,
            },
        )
        if self.control_plane is not None:
            self.control_plane.update_environment_metadata(
        ...
        return RestoreRecord(
            backup_id=backup_id,
            environment_id=raw["environment_id"],
            target=isolated_target.database_target,
            provider=result["provider"],
            verified=True,
            rto_seconds=max(0, int(elapsed)),
...
def select_backup_provider(
    *, test_only: bool, state_path: str | Path | None = None
) -> LocalBackupProvider:
    if not test_only:
        raise RuntimeError(
            "no production backup provider is configured; local adapter requires --test-only"
        )
    return LocalBackupProvider(state_path)


def record_mapping(
    record: BackupRecord | VerificationResult | RestoreRecord,
) -> dict[str, Any]:

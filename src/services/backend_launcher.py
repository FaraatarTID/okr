...
subprocess when running in managed cloud environments or when `OKR_BACKEND_API_URL`
is set to 'auto'. It also handles the case where the URL points to localhost,
which in managed cloud runtime means the backend should be self-hosted.
"""
...
# Sentinel to prevent re-running the launcher on every app rerun.
# Some app runtimes re-execute the script on interaction; we only want to
...
def _find_repo_root() -> str | None:
    """Walk up from this file's location to find repo root (contains backend_app/)."""
    file_dir = os.path.dirname(os.path.abspath(__file__))
    # backend_launcher.py lives at: <repo>/src/services/
...
def ensure_backend_running() -> bool:
    ...
    2. ``OKR_BACKEND_API_URL`` is empty AND we are in managed cloud runtime.
    3. ``OKR_BACKEND_API_URL`` points to ``localhost``/``127.0.0.1`` AND we
       are in managed cloud runtime (there is no external process to rely on).
    ...
    if want_auto or want_auto_cloud_empty:
        ...
        # Localhost URL detected on managed cloud runtime — parse host/port from URL.
    ...
    # We include repo root in PYTHONPATH so `backend_app` and shared `src` resolve.
    env = os.environ.copy()
    repo_root_str = str(repo_root)

    current_pp = env.get("PYTHONPATH", "")
    new_pp_parts = [repo_root_str]
    if current_pp:
    ...
    # On managed cloud runtime the env should be set explicitly to avoid

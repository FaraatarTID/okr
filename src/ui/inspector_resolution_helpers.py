...
def resolve_objective_scoring_section(
    *,
    ...
    # Runtime rollup is always weighted by KR weights; score_mode stays for
    # backward-compatible payload/UI contracts.
    weighted = True
    ...
    *,
    ...
    calc_score = float(
        calculate_kr_score_fn(
            ...
            target=values["new_target"],
            start=values["new_start"],
            metric_type=values["new_metric_type"],
        )
    )
    calc_p = max(0, min(100, int(round(calc_score * 100))))
    if not has_children:

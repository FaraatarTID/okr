...
def render_gantt_chart(
    ...
    fig.update_yaxes(
        autorange="reversed"
    )  # Waterfall top-down means list starts at top
    fig.update_layout(
        ...
        yaxis=dict(tickmode="array", tickvals=df["TaskUnique"], ticktext=df["Task"]),
    )

    # Add today line
    fig.add_vline(
        x=now, line_width=1, line_dash="dash", line_color="red", annotation_text="Today"
    )
    ...
    # Legend / Tips
    st.caption(
        "💡 **Tip:** Bars shown in faded colors indicate projected deadlines (Next Day) where no specific deadline was set."
    )

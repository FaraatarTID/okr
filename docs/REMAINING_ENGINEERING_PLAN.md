...
| --- | --- | --- | --- | --- | --- |
| A1 | `scripts/deploy_saas_release.py` is broken in every mode. It is the only lifecycle CLI missing the interpreter bootstrap guard. | Executed: `ModuleNotFoundError: No module named 'src'` (exit 1). `provision_`/`backup_`/`restore_`/`migrate_` all carry the guard at lines 10-11. | Add `if __package__ in {None, ""}: sys.path.insert(0, str(Path(__file__).resolve().parents[1]))`. | `python scripts/deploy_saas_release.py --help` exits 0. A subprocess test drives provision, deploy release-1, deploy release-2, rollback to release-1, and asserts the rolled-back state plus an audit event. | S |
| A2 | The restore path is unreachable, even with `--test-only`. | `RestoreManager` requires a pre-registered target; `src/saas/backup_operations.py:207` `register_target` has no non-test caller (only `tests/test_saas_backup_operations.py:152,351,400,411`). The restore CLI exposes `--isolated-target` but no registration path. | Add explicit target registration as a `register-target` subcommand in `scripts/restore_saas_environment.py` over the existing `register_target()`, and pass target resolution into the manager instead of relying on a global registry. | A CLI-driven restore into a registered isolated target completes. A `live` target is rejected. An unregistered target still fails closed. | S-M |

...
| C4 lint half | Landed | `eslint.config.mjs` is a flat config covering both workspaces, root `npm run lint` and `just lint` run it, and a `JavaScript lint gate` step early in `spa-quality` keeps it honest. `no-explicit-any` and `no-floating-promises` are `error`; `no-unused-vars` (19) and `exhaustive-deps` (18) are `warn`, so the 37 remaining findings are visible debt rather than a failing gate. The very first run found the two dead imports the gate exists to catch, including one this session's own C8 left in `useDeepLinkCycleBootstrap.ts`. |

...
| C4 lint half | Blocked | The lint gate cannot be wired from this environment: `node_modules` is owned by a different account and is not writable, so the ESLint toolchain cannot be installed and `package-lock.json` cannot be regenerated. Adding the devDependencies to `package.json` without the matching lockfile would break `npm ci` in CI, so nothing was half-wired. The config and the measured rule levels are prepared and recorded below, and the two dead imports the gate is meant to catch were removed by hand instead. |
| C4 lint evidence | Measured, not yet landed | Against the intended flat config over `spa-web/src` and `spa-bff/src`, 131 files. Measured twice, on ESLint 9.39.5 and again on the supported line (ESLint **10.11.0**, `@eslint/js` 10.0.1, `typescript-eslint` 8.70.0, `eslint-plugin-react-hooks` 7.1.1); ESLint 9 is now EOL, so the 10.x numbers are the ones to build on. **`no-explicit-any` has zero violations** on both, so it can be `error` immediately at no cost. Warnings to start at `warn`: `no-unused-vars` 19 and `exhaustive-deps` 18. Errors, each a single small site, to be fixed so the gate lands green: `no-floating-promises` 1 (`useSnapshotLifecycle.test.ts:135`), `no-useless-escape` 2 (`shellDateUtils.ts:17`, `api/http.ts:134`), `no-require-imports` 1 (`spa-bff/src/config.ts:172`), and `preserve-caught-error` 1 (`api/http.ts:200`), the last being a rule new in ESLint 10's recommended set. This corrects the staging this phase recommended in two places: `no-explicit-any` is free rather than noisy, and `no-floating-promises` has one pre-existing instance rather than enough debt to force `warn`. |

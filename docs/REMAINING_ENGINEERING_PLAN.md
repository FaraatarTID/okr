...
| F8 - Ten scripts are referenced nowhere, three of them verification gates | Open | Measured by searching every workflow, the justfile, every script and every document for each script's module stem, so both the `scripts/foo.py` and `python -m scripts.foo` invocation forms are covered. Ten scripts appear nowhere: `capture_compose_resources`, `compare_topology_evidence`, `db_tcp_probe`, `generate_baseline_migration`, `render_k8s_release`, `required_insert_columns`, `seed_performance_fixture`, and — the part that matters — `verify_helper_integrity`, `verify_module_export_contracts` and `verify_observability_readiness`. The first group are operator tools and generators that legitimately need arguments or a target, so being unreferenced is defensible for them. The last three are verification gates that pass when run by hand (`[PASS] Helper integrity checks passed for targeted modules`, `[PASS] Module export contract checks passed`, `OBSERVABILITY READINESS CHECK PASSED`) and that nothing invokes, so they cannot fail and nothing is checked. This is the same family as F4: a gate that is green because it does not run. It needs a deliberate decision per script — wire it into a workflow or a justfile recipe, or delete it — and is left open rather than wired up blind, since adding three new required checks to CI is a policy change with its own consequences. Note for whoever does it: run these via `python -m scripts.X`, not `python scripts/X.py`, because several import siblings as `scripts.*` and the direct-script form puts `scripts/` rather than the repository root on `sys.path`. Verified: `python -m scripts.verify_observability_readiness` passes while `python scripts/verify_observability_readiness.py` is the form that must be avoided. |

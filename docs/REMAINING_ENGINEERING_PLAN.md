...
| C4 lint evidence | Measured, not yet landed | Against the intended flat config (ESLint 9.39.5, `typescript-eslint` 8.70.0, `eslint-plugin-react-hooks` 7.1.1) over `spa-web/src` and `spa-bff/src`, 131 files: `no-unused-vars` 21, `exhaustive-deps` 18, `no-useless-escape` 2, `no-require-imports` 1, `no-floating-promises` 1, and **`no-explicit-any` 0**. That measurement changes the staging this phase recommended: `no-explicit-any` can be `error` immediately at no cost, and `no-floating-promises` has a single pre-existing instance in a test file rather than the debt that was expected, so both are cheap to make blocking; `no-unused-vars` (21) and `exhaustive-deps` (18) are the two that must start at `warn`. |

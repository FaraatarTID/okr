...
- [ ] `OKR_BACKEND_SECURITY_STATE_BACKEND` در production روی `database` یا `redis` تنظیم شده است.
- [ ] اگر backend روی `redis` است، مقدار `OKR_BACKEND_SECURITY_STATE_REDIS_URL` تنظیم شده است.
...
Phase 3 checks (job quotas)
- [ ] مقادیر quota/backoff برای `/v1/jobs` در env بازبینی شده‌اند:

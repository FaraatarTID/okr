...
| 14. Monorepo and package management tooling | IMPLEMENTED | `pyproject.toml`; `uv.lock`; `package.json`; `package-lock.json`; `spa-bff/package.json`; `spa-bff/package-lock.json`; `spa-web/package.json`; `scripts/check_import_boundaries.py`; `.github/workflows/ci.yml`; `deploy/docker/Dockerfile`; `spa-bff/tsconfig.json` | 2026-08-31: workspace manifests, lockfiles, CI adoption, boundary gate, Docker runtime adoption, source-aware build caches, Docker build validation, Linux optional-dependency coverage, and hoisted-tooling compatibility added | uv and npm now provide locked dependency/workspace boundaries across quality, deploy-build, e2e, and Docker runtime paths; full task-graph orchestration and external deployment review remain deferred. |
| 15. Frontend dependency security | IMPLEMENTED | `spa-web/package.json`; `spa-web/package-lock.json`; `spa-bff/package.json`; `spa-bff/package-lock.json`; `package-lock.json`; `spa-web/vitest.config.ts`; `spa-web/tsconfig.json` | 2026-08-31: upgraded Vitest, Next.js, and esbuild handling; compatibility checks passed | Critical and high advisories are cleared; SPA and BFF tests/builds/typechecks pass. One low development-only esbuild advisory remains in the root audit due to a nested Vite constraint. |

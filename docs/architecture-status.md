...
|---|---|---|---|---|
| 1. Typed API contracts / OpenAPI codegen | CLOSED | `scripts/export_openapi.py` + `scripts/check_openapi_drift.py` (CI gate); artifact `spa-web/src/lib/api/openapi.json`; generated types `spa-web/src/lib/api/generated/schema.d.ts` via `npm run gen:api`; typed helper `backend-schema.ts` + tests; generated request/response contracts consumed by `spa-web/src/lib/api/atlas.ts`, `useAtlasModeData.ts`, and `useAdminResources.ts` | 2026-08-26: break-a-payload drift drill completed; 2026-08-31: Atlas/read-query request and response adoption added | The gate caught the deliberate contract break and the restored schema passed. The read-query response envelope preserves unknown fields while exposing typed sections for ritual, KR, timeline, and shared resource workflows. |
...
| 9. Add strategy and fallback observability | VERIFIED | `backend_app/data_access_mode.py`: request-local effective strategy, resolver state, and fallback reason; `src/observability_metrics.py`: bounded strategy, fallback-reason, and resolver-state counters; `backend_app/observability_http.py`: request metrics attribution; tests in `tests/test_data_access_mode.py` and `tests/test_backend_observability.py` | 2026-08-31: focused attribution tests passed in CI (`python -m pytest -q tests/test_data_access_mode.py tests/test_backend_observability.py`) | Focused attribution tests pass; live failure/recovery verification remains pending. |
...
| 14. Monorepo and package management tooling | VERIFIED | `pyproject.toml`; `uv.lock`; `package.json`; `package-lock.json`; `spa-bff/package.json`; `spa-bff/package-lock.json`; `spa-web/package.json`; `scripts/check_import_boundaries.py`; `.github/workflows/ci.yml`; `deploy/docker/Dockerfile`; `spa-bff/tsconfig.json` | 2026-08-31: hosted CI passed after workspace install, pip-tooling, optional Rollup, and hoisted Vitest fixes | uv and npm now provide verified locked dependency/workspace boundaries across quality, deploy-build, e2e, and Docker runtime paths; full task-graph orchestration and external deployment review remain deferred. |
| 15. Frontend dependency security | VERIFIED | `spa-web/package.json`; `spa-web/package-lock.json`; `spa-bff/package.json`; `spa-bff/package-lock.json`; `package-lock.json`; `spa-web/vitest.config.ts`; `spa-web/tsconfig.json` | 2026-08-31: hosted CI passed after upgraded Vitest, Next.js, and esbuild handling | Critical and high advisories are cleared; SPA and BFF tests/builds/typechecks pass. One low development-only esbuild advisory remains in the root audit due to a nested Vite constraint. |
...
| 2026-08-31 | 10 | VERIFIED | Added deterministic failure-mode recovery harness in `tests/test_data_access_failure_recovery.py` and ran `python -m pytest -q tests/test_data_access_failure_recovery.py` (`3 passed`). |
| 2026-08-31 | 11 | IMPLEMENTED | Added `scripts/verify_deploy_readiness.py` gate to `.github/workflows/docker-deploy.yml` with backend/BFF/web + running service checks. |
| 2026-08-31 | 12 | IMPLEMENTED | Added `scripts/verify_migration_lint.py` gate and wired it into `ci.yml` before migration upgrade. |
| 2026-08-31 | 9 | VERIFIED | Ran `python -m pytest -q tests/test_data_access_mode.py tests/test_backend_observability.py` (`16 passed`) to verify strategy/fallback attribution behavior. |

...
| C4 - Lint gate and `spa-bff` typecheck | IMPLEMENTED | [REMAINING_ENGINEERING_PLAN.md](REMAINING_ENGINEERING_PLAN.md); `eslint.config.mjs`, root `npm run lint`, `just lint`, and the `JavaScript lint gate` step in `spa-quality` | 2026-09-20 | Both halves are done, so `just lint` and `just typecheck` now cover both JS packages. The lint gate runs over 131 files with 0 errors and 37 warnings: `no-explicit-any` and `no-floating-promises` are `error`, while `no-unused-vars` (19) and `exhaustive-deps` (18) stay `warn` as visible debt. The five error-level sites were fixed rather than downgraded, and the regex and `cause` changes were proven behaviour-preserving against 22 inputs and a non-enumerability check before landing. |

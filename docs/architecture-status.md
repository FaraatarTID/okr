...
| C4 - Lint gate and `spa-bff` typecheck | PARTIAL - typecheck IMPLEMENTED, lint gate blocked | [REMAINING_ENGINEERING_PLAN.md](REMAINING_ENGINEERING_PLAN.md); `spa-bff/package.json` has a `typecheck` script, root `typecheck` calls it, and `spa-quality` runs it | 2026-09-20 | The `spa-bff` typecheck half is done and covered by CI, so `just typecheck` covers both JS packages. The lint half is blocked by the environment rather than by design: `node_modules` here is owned by a different account and is not writable, so the ESLint toolchain cannot be installed and `package-lock.json` cannot be regenerated. Wiring `package.json` without that lockfile would break `npm ci` in CI, so nothing was half-wired. Rule levels were measured against the intended config over 131 files, on ESLint 10.11.0 (`no-explicit-any` 0, `no-floating-promises` 1, `no-useless-escape` 2, `no-require-imports` 1, `preserve-caught-error` 1, `exhaustive-deps` 18, `no-unused-vars` 19) and are recorded in the plan. The two dead imports the gate is meant to catch, `RitualModePanel.tsx`'s `rtlStyle` and one this session's C8 left in `useDeepLinkCycleBootstrap.ts`, were removed by hand. |

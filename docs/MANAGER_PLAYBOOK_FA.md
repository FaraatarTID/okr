...
## مرز مفهومی: Goal، Objective، KR، Initiative

کیفیت راهبری مدیر وابسته به تفکیک دقیق این سه سطح است:
- `Goal`: روایت استراتژیکِ چرایی، جهت حرکت، و تغییر موردنظر.
- `Objective`: تعهد خروجی استراتژیک چرخه (چه چیزی باید تغییر کند).
- `KR`: خط اثبات عددی با baseline و target (از کجا بفهمیم تغییر رخ داده).
- `Initiative/Task`: برنامه اجرای کار (چطور کار را انجام می‌دهیم)، نه شواهد اثبات.

کنترل کیفیت مدیر:
1. اگر چیزی که Objective نامیده شده فقط یک خط متریک است، باید به KR بازنویسی شود.
2. اگر چیزی که KR نامیده شده روایت کلی و بدون baseline/target است، باید به Objective بازنویسی شود.
3. اگر check-in فقط فعالیت را گزارش کند و حرکت متریک نداشته باشد، به‌عنوان شواهد KR پذیرفته نشود.
4. اگر KR به‌صورت لیست اقدام نوشته شده باشد، آن را به initiative/task + KR قابل اندازه‌گیری تفکیک کنید.

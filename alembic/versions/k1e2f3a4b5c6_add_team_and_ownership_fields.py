        ...
        if "team_id" not in existing_user_cols:
            with op.batch_alter_table("user", schema=None) as batch_op:
                ...
                batch_op.create_index(
                    batch_op.f("ix_user_team_id"), ["team_id"], unique=False
                )
        ...
        if table in existing_tables:
            ...
            with op.batch_alter_table(table, schema=None) as batch_op:
                ...
                if "team_id" not in existing_cols:
                    ...
                    batch_op.create_index(
                        batch_op.f(f"ix_{table}_team_id"), ["team_id"], unique=False
                    )
                    batch_op.create_foreign_key(
                        f"fk_{table}_team", "team", ["team_id"], ["id"]
                    )
                ...
                if (
                    table in ["objective", "key_result"]
                    and "weight" not in existing_cols
                ):
                ...
                if table != "goal" and "owner_id" not in existing_cols:
                    ...
                    batch_op.create_index(
                        batch_op.f(f"ix_{table}_owner_id"), ["owner_id"], unique=False
                    )
                    batch_op.create_foreign_key(
                        f"fk_{table}_owner", "user", ["owner_id"], ["id"]
                    )

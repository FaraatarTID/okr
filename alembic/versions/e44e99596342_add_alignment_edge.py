...
def upgrade() -> None:
    ...
    if "alignment_edge" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "alignment_type", sa.String(), nullable=False, server_default="SUPPORTS"
            ),
        ...
        with op.batch_alter_table("alignment_edge", schema=None) as batch_op:
            batch_op.create_index(
                "ix_alignment_edge_child_id", ["child_id"], unique=False
            )
            batch_op.create_index(
                "ix_alignment_edge_parent_id", ["parent_id"], unique=False
            )
            batch_op.create_index(
                "ix_alignment_parent_child", ["parent_id", "child_id"], unique=True
            )

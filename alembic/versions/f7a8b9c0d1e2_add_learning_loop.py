...
def upgrade() -> None:
    ...
    if "experiment" not in existing_tables:
        ...
        with op.batch_alter_table("experiment", schema=None) as batch_op:
            batch_op.create_index(
                "ix_experiment_key_result_id", ["key_result_id"], unique=False
            )
            batch_op.create_index("ix_experiment_cycle_id", ["cycle_id"], unique=False)
            batch_op.create_index(
                "ix_experiment_kr_status", ["key_result_id", "status"], unique=False
            )
            batch_op.create_index(
                "ix_experiment_cycle_status", ["cycle_id", "status"], unique=False
            )
    ...
    if "retro_experiment_outcome" not in existing_tables:
        ...
        with op.batch_alter_table("retro_experiment_outcome", schema=None) as batch_op:
            batch_op.create_index(
                "ix_retro_experiment_outcome_retrospective_id",
                ["retrospective_id"],
                unique=False,
            )
            batch_op.create_index(
                "ix_retro_experiment_outcome_experiment_id",
                ["experiment_id"],
                unique=False,
            )
            batch_op.create_index(
                "ux_retro_experiment",
                ["retrospective_id", "experiment_id"],
                unique=True,
            )
    ...
    if "check_in" in existing_tables:
        ...
        check_in_indexes = {
            idx.get("name")
            for idx in inspector.get_indexes("check_in")
            if idx.get("name")
        }
        ...
        with op.batch_alter_table("check_in", schema=None) as batch_op:
            ...
            if "special_cause_note" not in check_in_columns:
                batch_op.add_column(
                    sa.Column("special_cause_note", sa.String(), nullable=True)
                )
